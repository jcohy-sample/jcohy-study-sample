...
8.mysql 安装完成之后,在 `/var/log/mysqld.log` 文件中给 `root` 生成了一个默认密码.通过下面的方式找到 `root` 默认密码,然后登录 `mysql` 进行修改:
...
1. https://software.jcohy.com/linux/redis-5.0.5.tar.gz[下载] redis 安装包
...
3.注意这种方式启动 redis 使用的是默认配置.也可以通过启动参数告诉 redis 使用指定配置文件使用下面命令启动.
...
1.使用 docker 安装 http://www.runoob.com/docker/docker-install-redis.html[redis]
...
+
image::https://jcohy-study.oss-cn-beijing.aliyuncs.com/images/jenkins3.jpg[]
...
image::https://jcohy-study.oss-cn-beijing.aliyuncs.com/images/jenkins4.jpg[]
