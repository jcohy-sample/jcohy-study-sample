...
| ElasticSearch  | CentOS Linux release 7.6 | release | 无 | 无
|===
...
+
. 解决方法1: 进入: `ip:端口/pluginManager/advanced`  修改升级站点中的地址,把 `https:` 改成 `http:` 并且点那个提交按钮.
+
...
+
造成此问题的原因是因为此符文所在的文件或者依赖不存在,主要还是因为jar包因为出现问题,出现的原因主要有以下两种情况.

. 此项目所依赖的服务版本未更新到最新版本或者未上传发布到私服中央仓库
+
解决方法: 使用Jenkins重新构建项目所依赖的微服务,将其生成的jar包上传至仓库中.
+
...
此问题是由于在执行脚本时,用户权限不足所造成的.需要在Jenkins的脚本文件中为当前命令添加所需要的权限
