...
public class TestTransaction {
	...
	public void test1() {
		transactions.stream().filter((x) -> x.getYear() > 2011)
				.sorted((e1, e2) -> Integer.compare(e1.getValue(), e2.getValue())).forEach(System.out::println);
	}
	...
	@Test
	public void test3() {
		transactions.stream().filter((x) -> x.getTrader().getCity().equals("Cambridge")).map(Transaction::getTrader)
				.sorted((e1, e2) -> e1.getName().compareTo(e1.getName())).distinct().forEach(System.out::println);
	}
	...
	@Test
	public void test6() {
		Integer cambridge = transactions.stream().filter((t) -> t.getTrader().getCity().equals("Cambridge"))
	...
	@Test
	public void test7() {
		Optional<Integer> cambridge = transactions.stream().filter((t) -> t.getTrader().getCity().equals("Cambridge"))
	...
	public void test8() {
		Optional<Integer> cambridge = transactions.stream().filter((t) -> t.getTrader().getCity().equals("Cambridge"))
				.map(Transaction::getValue).min(Integer::compare);
		System.out.println(cambridge);
	}
