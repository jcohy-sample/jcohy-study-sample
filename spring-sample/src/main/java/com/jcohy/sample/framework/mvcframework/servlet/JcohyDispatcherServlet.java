package com.jcohy.sample.framework.mvcframework.servlet;

import com.jcohy.sample.framework.mvcframework.HandlerMapping;
...
public class JcohyDispatcherServlet extends HttpServlet {
	...
	private void doDispatcher(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		for (Map.Entry<String, String[]> params : parameterMap.entrySet()) {
			...
			if (!handlerMapping.getParamIndexmapping().containsKey(params.getKey())) {
				continue;
			}
	...
	private void initHandlerMapping() {
		...
		for (Map.Entry<String, Object> entry : ioc.entrySet()) {
			...
			for (Method method : methods) {
				if (!method.isAnnotationPresent(JcohyRequestMapping.class)) {
					continue;
				}
	...
	private void doAutowired() {
		...
		for (Map.Entry<String, Object> entry : ioc.entrySet()) {
			...
			for (Field field : fields) {
				if (!field.isAnnotationPresent(JcohyAutowired.class)) {
					continue;
				}
