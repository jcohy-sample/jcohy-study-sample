...
public class ProducerMessage implements RabbitTemplate.ConfirmCallback, RabbitTemplate.ReturnCallback {
    ...
    /**
     * 如果发送到交换器都没有成功，比如删除了交换器，ack 返回值为 false
     ...
     *
     * 如果 ConfirmCallback 中 ack 返回 true，这里需要注意的是，当 exchange 没有找到匹配队列，ack 也会返回 true，说明消息发到 exchange 中成功，这个时候需要查看 ReturnCallback 中有没有发生异常，如果消息正常投递，
     * 可以通过 correlationData 删除 redis 或者数据库中的本地消息，否则也需要做异常处理。
     *
    ...
    /**
     * 如果发送到交换机成功，但是没有匹配的队列，则会触发这个回调
     * 开启 publisher-returns，发送到交换器成功，但是没有匹配的队列会触发，这样消息也会丢失。
     */
    @Override
